...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  return <>
    <header>
      ...
      {open && <div style={{ position:'absolute', top:'calc(100% + 8px)', left:12, right:12, background:'#fff', border:'1px solid var(--line)', borderRadius:14, padding:10, boxShadow:'0 15px 35px rgba(0,0,0,.12)', zIndex:100 }}>
        {['/','/create','/how-it-works','/help','/about'].map((path) => <Link key={path} style={{display:'block',padding:12}} to={path} onClick={() => setOpen(false)}>{path==='/'?'Home':path==='/create'?'Create group':path==='/how-it-works'?'How it works':path==='/help'?'Help & FAQ':'About'}</Link>)}
      </div>}
    </header>
    {children}
